...
public class AnalyticsService {
        ...
        public java.util.List<Map<String, Object>> getCrimesByCategory(int days)
                ...
                return categoryCounts.entrySet().stream()
                                .map(entry -> {
                                        ...
                                        java.util.List<com.crimenet.model.CrimeReport> categoryReports = allReports
                                                        .stream()
                                                        .filter(r -> entry.getKey().equals(r.getCategory()))
                                                        .filter(r -> r.getCreatedAt() != null && r.getCreatedAt()
                                                                        .toDate().getTime() >= cutoffTime)
                                                        .collect(java.util.stream.Collectors.toList());

                                        item.put("pending", categoryReports.stream()
                                                        .filter(r -> "PENDING".equals(r.getStatus())).count());
                                        item.put("inProgress",
                                                        categoryReports.stream().filter(
                                                                        r -> "IN_PROGRESS".equals(r.getStatus())
                                                                                        || "INVESTIGATING".equals(
                                                                                                        r.getStatus()))
                                                                        .count());
        ...
        public Map<String, Object> getUserStats(String userId) throws ExecutionException, InterruptedException {
                ...
                stats.put("activeCases", userReports.stream().filter(r -> "PENDING".equals(r.getStatus())
                                || "IN_PROGRESS".equals(r.getStatus()) || "INVESTIGATING".equals(r.getStatus()))
                                .count());
